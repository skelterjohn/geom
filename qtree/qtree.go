...
func New(cfg *Config, height int, bounds *geom.Rect) (me *Tree) {
	me = &Tree{
		height:    height,
		cfg:       cfg,
		Bounds:    bounds,
...
func (me *Tree) insertSubTrees(element geom.Bounded) (inserted bool) {
	for i, t := range me.Subtrees {
		if t == nil {
			...
			t = New(me.cfg, me.height-1, &subbounds)
			me.Subtrees[i] = t
		}
...
func (me *Tree) Collect(bounds *geom.Rect, collection map[geom.Bounded]bool) (found bool) {
	str := ""
	...
	for _, t := range me.Subtrees {
		...
		found = t.Collect(bounds, collection) || found
	}
